...
#SparseAttn for Bigbird
class SparseAttention(nn.Module):

    def __init__(self, hidden_size, n_splits, block_splits=8):
        super().__init__()

        self.hidden_size = hidden_size
        self.n_split = n_splits

        self.block_splits = block_splits

        self.Q_linear = nn.Linear(hidden_size/n_splits, hidden_size/n_splits, bias=False)
        self.K_linear = nn.Linear(hidden_size/n_splits, hidden_size/n_splits, bias=False)
        self.V_linear = nn.Linear(hidden_size/n_splits, hidden_size/n_splits, bias=False)
    ...
    def forward(self, Q, K, V, mask=None, dk=64):
        ...
        # |mask| = (batch_size * n_splits, m, m)

        # block attention
        QWs = self.Q_linear(Q).split(self.block_splits, dim=-1)
        KWs = self.K_linear(K).split(self.block_splits, dim=-1)
        VWs = self.V_linear(V).split(self.block_splits, dim=-1)
        # |QW_i| = |KW_i| = |VW_i| = ((batch_size * n_splits), m, (hidden_size / n_splits) / block_split)

        window_w = self.window_attn(QWs, KWs, VWs, mask, dk)

        random_w = self.random_attn(QWs, KWs, VWs, mask, dk)

        # |Q| = |K| = |V| = ((batch_size * n_splits) * block_split, m, (hidden_size / n_splits) / block_split)

        # w = attention energy
        w = torch.bmm(Q, K.transpose(1, 2))
        # |w| = (batch_size * n_splits, m, m)
        ...
        # |w| = (batch_size * n_splits, m, m)
        # |V| = (batch_size * n_splits, m, hidden_size / n_splits)
        c = torch.bmm(w, V)
        # |c| = (batch_size * n_splits, m, hidden_size / n_splits)

        return c

    def window_attn(self, QWs, KWs, VWs, mask=None, dk=64): 
        # |QW_i| = |KW_i| = |VW_i| = ((batch_size * n_splits), m, (hidden_size / n_splits) / block_split)

        # Key를 rolling, batch 방향으로 잘린 것을 순서대로 섞으면 됨
        KWs = self._sliding(KWs)
    ...
    def random_attn(self, r=8):
        pass

    def _sliding(self, KWs, w=3):

        #KWs의 복사본을 w 갯수만큼 만들기

        KW_list_left = KWs
        KW_list_left_first = KW_list_left[0]
        for idx in range(self.block_splits):
            if idx == self.block_splits:
                KW_list_left[idx] = KW_list_left_first
            else:
                KW_list_left[idx] = KW_list_left[idx + 1]
        
        KW_list_right = KWs
        KW_list_right_contain = None
        for idx in range(self.block_splits):
            KW_list_right_contain = KW_list_right[idx + 1]
            KW_list_right[idx + 1] = KW_list_right[idx]
    ...
    # KT에서는 어떻게 적용할지 생각해보기, 각 시퀀스의 가장 마지막에 있는 것(마스크가 아닌)이 가장 중요하지 않을까?
    # def global_attn(self):
    #     pass
...
class MultiHead(nn.Module):
    ...
    def __init__(self, hidden_size, n_splits):
        ...
        self.attn = SparseAttention(hidden_size, n_splits)
