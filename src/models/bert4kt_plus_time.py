...
class Attention(nn.Module):
    ...
    def forward(self, Q, K, V, mask=None, dk=64):
        ...
        Q_f = self._forgetting_def(Q)
        K_f = self._forgetting_def(K)

        # w = attention energy
        w = torch.bmm(Q_f, K_f.transpose(1, 2))
    ...
    def _forgetting_def(self, Q, theta=0.01):

        batch_size = Q.size(0)
        sq_size = Q.size(1)
        emb_size = Q.size(2)

        forgetting_score = torch.arange(emb_size).unsqueeze(0)
        forgetting_score = forgetting_score.expand(batch_size, sq_size, emb_size).float()
        forgetting_score = self.logsoftmax(forgetting_score) * theta
        forgetting_score = forgetting_score.to(self.device)

        forgetting_value = Q + (Q * forgetting_score)

        return forgetting_value


class MultiHead(nn.Module):

    def __init__(self, hidden_size, n_splits, device):
        super().__init__()
...
class EncoderBlock(nn.Module):
    ...
    def __init__(
        ...
        self.attn = MultiHead(hidden_size, n_splits, device)
...
class Bert4ktPlusTime(nn.Module):
