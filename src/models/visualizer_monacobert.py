...
'''TODO LIST
- Objective 1 : Filter pattern analysis
    [] testset -> extract filter pattern per layers
    [] filter_pid = {
            'num_pid' : {
                'num_layer' : list = [torch.tensor(8, 9)]
            }
        }
    [] filter_con = {
            'num_con' : {   
...
class MonotonicConvolutionalMultiheadAttentionVis(nn.Module):
    # hidden % n_splits == 0
    def __init__(self, hidden_size, n_splits, dropout_p, head_ratio=2, conv_kernel_size=9):
        ...
        # this is for the save the result
        self.filters = None
...
class EncoderBlockVis(nn.Module):
    ...
    def __init__(
        ...
        self.attn = MonotonicConvolutionalMultiheadAttentionVis(hidden_size, n_splits, dropout_p)
