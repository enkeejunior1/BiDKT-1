...
class MonotonicConvolutionalMultiheadAttentionPara(nn.Module):
    # hidden % n_splits == 0
    def __init__(self, hidden_size, n_splits, dropout_p, head_ratio=2, conv_kernel_size=9):
        ...
        self.query_conv_attn_layer = nn.Linear(hidden_size, self.all_head_size, bias=False) # 512 -> 256
    ...
    def forward(self, Q, K, V, mask=None):
        ...
        mixed_query_conv_attn_layer = self.query_conv_attn_layer(Q)
        ...
        conv_attn_layer = torch.multiply(mixed_key_conv_attn_layer, mixed_query_conv_attn_layer)
...
class EncoderBlock(nn.Module):
    ...
    def __init__(
        ...
        self.attn = MonotonicConvolutionalMultiheadAttentionPara(hidden_size, n_splits, dropout_p)
...
class MonaConvBert4ktPlusDiffPara(nn.Module):
